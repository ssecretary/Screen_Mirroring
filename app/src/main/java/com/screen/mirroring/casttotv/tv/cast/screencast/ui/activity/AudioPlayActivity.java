...
import static com.screen.mirroring.casttotv.tv.cast.screencast.Constant.AUDIO_PREFIX;
...
public class AudioPlayActivity extends AppCompatActivity implements DeviceConnectListener, Observer, OnItemClickListner {
    ...
    private BroadcastReceiver nextVideoDLNAPlay = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (binding != null && binding.seekBar.getProgress() > 0) {
                playNextVideo();
            }
        }
    };
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        queueListAdapter = new QueueListAdapter(this, true);
    ...
    public void initView() {
        ...
        Glide.with(this).load(Uri.parse("content://media/external/audio/albumart/" + video.getAlbumId())).apply(new RequestOptions().transform(new CenterCrop(), new RoundedCorners(18))
        ...
        binding.next.setOnClickListener(v -> {
            if (position < videoList.size() - 1) {
                binding.ivPlayPause.setImageResource(R.drawable.ic_pause_circle_white_80dp);
                position = position + 1;
                ...
                if (isChromeCastConnected && mediaQueueItem != null) {
                    ...
                    if (position < videoList.size()) {
                        video = videoList.get(position);
                        Glide.with(this).load(Uri.parse("content://media/external/audio/albumart/" + video.getAlbumId())).apply(new RequestOptions()
                                .diskCacheStrategy(DiskCacheStrategy.ALL).error(R.drawable.ic_music_placeholder))
                                .into(binding.thumbnailIcon);
                        binding.fileName.setText(video.getFileName());
                        castVideo();
                    }
        ...
        binding.seekBar.setOnSeekChangeListener(new OnSeekChangeListener() {
            ...
            public void onStopTrackingTouch(IndicatorSeekBar seekBar) {
                if (isDLNACastConnected) {
                    ...
                    Toast.makeText(getApplicationContext(), "Seek to " + seek, Toast.LENGTH_SHORT).show();
    ...
    public void castVideo() {
        ...
        if (!isConnected) {
            startActivity(new Intent(AudioPlayActivity.this, CastDeviceListActivity.class));
            return;
        } else {
            if (isChromeCastConnected) {
        ...
        if (isChromeCastConnected) {
            MediaInfo mediaInfo1 = webServerController.getAudioMediaInfo(video.getFilePath());
...
//            remoteMediaClient.load(mediaInfo1, true, 0);
            ...
            String mediaUrl = "http://" + ip1 + ":" + MediaServer.port + "/" + AUDIO_PREFIX + video.getId() + extension;
