...
package com.screen.mirroring.casttotv.tv.cast.screencast.browser;
...
public abstract class VideoContentSearch implements Runnable {
    ...
    public abstract void onVideoFound(String size, String type, String link, String name,
                                      String page, boolean chunked, String website, String width, String height, String thumbnail_url);
    ...
    private void addVideoToList(URLConnection uCon, String page, String title, String contentType) {
        try {
            ...
            if (host.contains("youtube.com") || (new URL(link).getHost().contains("googlevideo.com")
                ...
                if (host.contains("youtube.com")) {
                    ...
                    try {
                        ...
                        thumbnail_url = new JSONObject(jSonString).getString("thumbnail_url");
                        String html = new JSONObject(jSonString).getString("html");
                        link = html.substring(html.indexOf("src="), html.indexOf("?feature=oembed")).replace("src=", "").replace("\"", "");
                        link = link + "?autoplay=1";
                        width = html.substring(html.indexOf("width="), html.indexOf("height")).replace("width=", "").replace("\"", "");
                        height = html.substring(html.indexOf("height="), html.indexOf("src")).replace("height=", "").replace("\"", "");
            ...
            onVideoFound(size, type, link, name, page, chunked, website, width, height, thumbnail_url);
